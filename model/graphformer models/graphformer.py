...
class encoder(nn.Module):
    def __init__(self, encoder_dims: int, n_heads: int = 4, n_mha: int = 1, n_alignn: int = 4, n_gnn: int = 4, residual: bool = True, norm: bool = True, last: bool = False, seq_len: int = 256):
        super(encoder, self).__init__()
        assert encoder_dims % n_heads == 0
        assert n_heads >= 1
        assert n_gnn >= 1
        self.head_dim = encoder_dims // n_heads
        self.residual = residual
        self.norm = norm
        if last:
            mha_layers = [
                transformer.multiheaded(encoder_dims, self.head_dim, heads=n_heads, seq_len=seq_len)
                for _ in range(n_mha - 1)
            ]
            mha_layers.append(
                transformer.multiheaded(encoder_dims, self.head_dim, heads=n_heads, norm=(True, False), seq_len=seq_len)
            )
    ...
    def forward(self, g: dgl.DGLGraph, lg: dgl.DGLGraph, fg: dgl.DGLGraph, x: Tensor, y: Tensor, z: Tensor, f: Tensor):
...
class Graphformer(nn.Module):
    ...
    def forward(self, g: dgl.DGLGraph, lg: dgl.DGLGraph, fg: dgl.DGLGraph):
        ...
        for encdr in self.encoders:
            atom_attr, edge_attr, angle_attr, fc_attr = encdr(
                g, lg, fg, atom_attr, edge_attr, angle_attr, fc_attr
            )
