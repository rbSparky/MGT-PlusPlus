...
api_key="222ef6fdc43176f75927d4972da89a00d4e11b2d"
...
from model.bnpool import BNPool,Poolencoder,GraphPoolformer
            ...
            try:
                with torch.autocast(device_type=fabric.device.type, dtype=torch.float16, enabled=fabric.device.type == "cuda"):
                    with fabric.no_backward_sync(model, enabled=is_accumulating):
                        output, _, _, _, _,loss_d = model(g, lg, fg)
                        if loss_d is not None:
                            loss_d = loss_d["quality"] + loss_d["kl"]
                        loss = criterion(output, target) / args.n_cum + (loss_d if loss_d is not None else 0)
...
def main(args):
    ...
    policy = {Poolencoder, EdgeGatedGraphConv, multiheaded}
    ...
    model = GraphPoolformer(args=args)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--pooling',
        type=str,
        choices=['bnpool', 'avgpool'],
        default='avgpool', # Or 'bnpool' if you want it as default
        help='Type of graph pooling to use. Choose from "bnpool" or "avgpool".'
    )
    parser.add_argument(
        '--n_clusters',
        type=int,
        default=50,
        help='Maximum number of clusters for BNPool (if --pooling bnpool is used).'
    )
    parser.add_argument(
        '--n_particles',
        type=int,
        default=1,
        help='Number of particles for the Stick Breaking Process in BNPool.'
    )
    parser.add_argument(
        '--alpha_dp',
        type=float,
        default=10.0,
        help='Concentration parameter (alpha) for the Dirichlet Process prior in BNPool.'
    )
    parser.add_argument(
        '--sigma_k',
        type=float,
        default=1.0,
        help='Variance of the Gaussian prior for the cluster-cluster probability matrix in BNPool.'
    )
    parser.add_argument(
        '--mu_k',
        type=float,
        default=10.0,
        help='Mean of the Gaussian prior for the diagonal of the cluster-cluster probability matrix in BNPool.'
    )
    parser.add_argument(
        '--k_init',
        type=float,
        default=1.0,
        help='Initial value for the diagonal elements of the cluster-cluster probability matrix in BNPool.'
    )
    parser.add_argument(
        '--eta',
        type=float,
        default=1.0,
        help='Coefficient for the KL divergence loss term in BNPool.'
    )
    parser.add_argument(
        '--no_rescale_loss',
        action='store_false',
        dest='rescale_loss',
        help='Disable loss rescaling by the number of nodes in BNPool.'
    )
    parser.add_argument(
        '--rescale_loss', # This flag explicitly enables it, in case you change the default later
        action='store_true',
        dest='rescale_loss',
        help='Enable loss rescaling by the number of nodes in BNPool (default). DO NOT use with --no_rescale_loss.'
    )
    parser.set_defaults(rescale_loss=True) # Explicitly set default if neither flag is used
    parser.add_argument(
        '--no_balance_links',
        action='store_false',
        dest='balance_links',
        help='Disable balancing links in the adjacency matrix reconstruction loss in BNPool.'
    )
    parser.add_argument(
        '--balance_links', # This flag explicitly enables it
        action='store_true',
        dest='balance_links',
        help='Enable balancing links in the adjacency matrix reconstruction loss in BNPool (default). DO NOT use with --no_balance_links.'
    )
    parser.set_defaults(balance_links=True) # Explicitly set default
    parser.add_argument(
        '--no_train_k',
        action='store_false',
        dest='train_K',
        help='Do not train the cluster-cluster probability matrix K in BNPool.'
    )
    parser.add_argument(
        '--train_k', # This flag explicitly enables it
        action='store_true',
        dest='train_k',
        help='Train the cluster-cluster probability matrix K in BNPool (default). DO NOT use with --no_train_k.'
    )
    parser.set_defaults(train_k=True)
